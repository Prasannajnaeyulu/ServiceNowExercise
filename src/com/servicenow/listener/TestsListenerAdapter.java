package com.servicenow.listener;
...
import com.servicenow.util.ErrorUtil;



public class TestsListenerAdapter implements IInvokedMethodListener {

	Logger logger = Logger.getLogger(TestsListenerAdapter.class);
	public void afterInvocation(IInvokedMethod method, ITestResult result) {
		logger.info("Inside afterInvocation Method....");
		Reporter.setCurrentTestResult(result);
		
		if(!result.isSuccess()){
			try {
				logger.info("taking the screenshot...");
				AbstractPageObject.takescreenshot(method.getTestMethod().getMethodName()+System.currentTimeMillis()+".jpg");
			} catch (IOException e) {
